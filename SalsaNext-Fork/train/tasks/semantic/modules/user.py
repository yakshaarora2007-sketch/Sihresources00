...
class User():
  def __init__(self, ARCH, DATA, datadir, logdir, modeldir,split,uncertainty,mc=30):
    ...
    parser_module_name = "tasks.semantic.dataset.{}.parser".format(self.DATA["name"])
    spec = importlib.util.spec_from_file_location(parser_module_name, parser_path)

    parserModule = importlib.util.module_from_spec(spec)
    sys.modules[parser_module_name] = parserModule
