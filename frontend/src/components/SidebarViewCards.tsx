...
export const SidebarViewCards: React.FC<SidebarViewCardsProps> = ({
  ...
  return (
    <aside className="w-56 sm:w-60 xl:w-64 h-full bg-slate-900/95 border-r border-slate-800 flex flex-col shrink-0 overflow-hidden select-none backdrop-blur-md z-20">
      {/* Sidebar Compact Header */}
      <div className="px-3 py-1.5 border-b border-slate-800/80 bg-slate-900/50 shrink-0 flex items-center justify-between">
        <span className="text-[11px] font-bold text-slate-200 tracking-wider">
          LIVE PREVIEW (5x)
        </span>
        <span className="text-[9px] font-mono text-emerald-400 bg-emerald-950/80 border border-emerald-800/50 px-1 py-0.2 rounded">
          REAL-TIME
        </span>
      </div>

      {/* 5 Cards Container: Flex column dividing available height into 5 equal non-scrolling rows */}
      <div className="p-1.5 flex-1 min-h-0 flex flex-col gap-1.5 justify-between overflow-hidden">
        {VIEW_CARDS.map((card) => {
          ...
          return (
            <div
              ...
              className={`group flex-1 min-h-0 flex flex-col justify-between rounded-lg border transition-all duration-150 cursor-pointer p-1.5 overflow-hidden ${
                isSelected
            ...
            >
              ...
              <div className="flex items-center justify-between shrink-0 leading-none">
                ...
                {isSelected ? (
                  <span className="text-[8px] font-bold font-mono text-emerald-950 bg-emerald-400 px-1.5 py-0.5 rounded-full shadow">
                    ACTIVE
                  </span>
                ) : (
