...
export const TelemetryHUD: React.FC<TelemetryHUDProps> = ({ metrics, numCells }) => {
  ...
  return (
    <div className="absolute top-4 left-4 z-20 flex flex-col font-mono select-none">
      <div className="bg-slate-900/90 backdrop-blur-md border border-slate-700/60 rounded-lg shadow-2xl overflow-hidden transition-all duration-200 min-w-72">
        {/* Header Bar */}
        <div
          onClick={() => setIsExpanded(!isExpanded)}
          className="flex items-center justify-between px-3 py-2 bg-slate-800/80 cursor-pointer hover:bg-slate-800 transition border-b border-slate-700/50"
        >
        ...
        {/* Content Body */}
        {isExpanded && (
          <div className="p-3 text-xs space-y-2.5 text-slate-300">
            {/* FPS & Jitter */}
            <div className="grid grid-cols-2 gap-2 bg-slate-950/60 p-2 rounded border border-slate-800">
              <div>
                <span className="text-[10px] text-slate-400 block">ACTUAL FPS</span>
                <span className={`text-base font-bold ${metrics.actualFps >= 9.5 ? 'text-emerald-400' : 'text-amber-400'}`}>
                  {metrics.actualFps.toFixed(1)} <span className="text-[10px] text-slate-500 font-normal">FPS</span>
                </span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 block">AVG FPS (60f)</span>
                <span className="text-base font-bold text-slate-200">
                  {metrics.avgFps.toFixed(1)} <span className="text-[10px] text-slate-500 font-normal">FPS</span>
                </span>
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <span className="text-[10px] text-slate-400 tracking-wider">LATENCY BREAKDOWN</span>
